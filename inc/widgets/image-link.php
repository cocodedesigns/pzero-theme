...
class CCD_ImageBox_Widget extends WP_Widget {
 
    public function __construct() {
     
        parent::__construct(
            'ccd_icb__widget',
            __( 'Image Link', 'ccd_widget' ),
            array(
                'classname'   => 'ccd_icb_widget',
                'description' => __( 'Creates an image link to a page.', 'ccd_widget' ),
    ...
    public function widget( $args, $instance ) {    
        ...
        $att_med = wp_get_attachment_image_src( $attid, 'large' );
        echo $before_widget;
        ?>
        <div class="image-link-widget">
          <div class="il-image" style="background-image: url('<?php echo $att_med[0]; ?>');"></div>
          <h2 class="il-title"><?php echo $title; ?></h2>
          <a href="<?php echo get_permalink($link); ?>" class="il-link">View Page</a>
        </div>
        <?php
        echo $after_widget;
...
add_action( 'widgets_init', function(){
     register_widget( 'CCD_ImageBox_Widget' );
});